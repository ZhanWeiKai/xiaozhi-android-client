...
class AudioUtil {
  ...
  static Future<void> initPlayer() async {
    ...
    try {
      ...
      await _pcmPlayer!.initialize(sampleRate: SAMPLE_RATE, nChannels: CHANNELS);
  ...
  static Future<void> playOpusData(Uint8List opusData) async {
    ...
    // 部分上游（自建 worker 的 24k TTS）在每个 opus 包前加 16 字节帧头：
    //   [16B header][opus payload]，其中 header[14:16](big-endian) = opus 负载长度。
    // 直连 xiaozhi.me 是裸 opus（无头部）。这里自动检测并剥离——
